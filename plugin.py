...
"""
    ...
    def _get_config_wrapper(self, key: str, default: Any = None) -> Any:
        ...
        if top_key in manual_load_keys:
            try:
                ...
                # 尝试从组件注册表获取插件配置
                plugin_config = component_registry.get_plugin_config(self.plugin_name)
                if plugin_config:
                    # 支持点状路径访问
                    value = plugin_config
                    for k in key.split("."):
                    ...
                    logger.warning(f"无法从组件注册表获取插件 {self.plugin_name} 的配置")
                    return default
    ...
    async def on_plugin_loaded(self):
        ...
        # 插件配置文件会由插件系统自动管理，无需手动创建
        # 如果需要默认配置，应该通过config_schema定义或让用户自行配置
